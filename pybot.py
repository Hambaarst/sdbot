...
async def draw(
    ...
    for image in post_response_json["images"]:
        ...
        button2 = UpscaleButton(
            image=image, row=1, label=f"upscale {i}", style=discord.ButtonStyle.gray
        )
...
async def img2img(image, send_data):
    ...
    for item in post_response_json["images"]:
        ...
        button2 = UpscaleButton(
            image=item, row=1, label=f"upscale {i}", style=discord.ButtonStyle.gray
        )
        view.add_item(button)
        view.add_item(button2)
        i = i + 1
